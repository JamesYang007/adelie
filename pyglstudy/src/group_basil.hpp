...
#include <ghostbasil/optimization/group_basil_cov.hpp>
#include <ghostbasil/optimization/group_basil_naive.hpp>

namespace py = pybind11;
using namespace pybind11::literals; // to bring in the `_a` literal
namespace gl_cov = ghostbasil::group_lasso::cov;
namespace gl_naive = ghostbasil::group_lasso::naive;
...
inline py::dict convert_group_lasso_diagnostic(
    const gl_cov::GroupLassoDiagnostic& diag
)
{
    return py::dict(
...
inline py::dict convert_group_lasso_diagnostic(
    const gl_naive::GroupLassoDiagnostic& diag
)
{
    return py::dict(
        "time_strong_cd"_a=diag.time_strong_cd,
...
)
{
    ...
    for (size_t i = 0; i < diag.time_group_lasso.size(); ++i) {
        gl_diags.emplace_back(
            convert_group_lasso_diagnostic(diag.time_group_lasso[i])
        );
    }
    return py::dict(
...
)
{
    ...
    ghostbasil::group_lasso::transform_data(X, groups, group_sizes, n_threads, diag);
    py::dict d(
        "A_diag"_a=diag
    );
    return d;
