...
#include <grpglmnet_core/optimization/group_basil_base.hpp>
...
struct GroupBasilState
{
    ...
    )
    {
        ...
        grpglmnet_core::screen(abs_grad, lmda_prev_valid, lmda_next, 
        ...
        CUIType check_user_interrupt = CUIType())
{
    ...
    using lasso_pack_t = GroupElnetParamPack<A_t, value_t, index_t, bool_t>;
    ...
    while (1) 
    {
        ...
        if (do_early_exit && (rsqs.size() >= 3)) {
            ...
            if (check_early_stop_rsq(rsq_l, rsq_m, rsq_u, cond_0_thresh, cond_1_thresh)) break;
        }
