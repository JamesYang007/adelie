#pragma once
#include <Eigen/Core>
#include <grpglmnet_core/optimization/bisect.hpp>
...
/**
 * @brief 
 * Compute upper bound h_max >= 0 such that bcd_root_function(h_max) <= 0.
 * NOTE: if zero_tol > 0,
...
)
{
    ...
    const auto initial_f = [&]() {
        const value_t h_min = bcd_root_lower_bound(vbuffer1, v, l1);
        const auto h_max_out = bcd_root_upper_bound(vbuffer1, v, 0.0); // IMPORTANT: NEEDS GUARANTEE
        const value_t h_max = std::get<0>(h_max_out);

        value_t h;
        size_t iters_brent;
        brent(
            [&](auto x) { return bcd_root_function(x, vbuffer1, v, l1); },
            tol,
            brent_tol, 
            max_iters,
            h_min,
...
)
{
    ...
    const auto initial_f = [&]() {
        const value_t h_min = bcd_root_lower_bound(vbuffer1, v, l1);
        const auto h_max_out = bcd_root_upper_bound(vbuffer1, v, l1);
        const value_t h_max = std::get<0>(h_max_out);
        ...
        if (h_max - h_min <= 1e-1) {
            ...
            const auto ada_bisect = [&]() {
                ...
                fh = bcd_root_function(h_cand, vbuffer1, v, l1);
            };
...
)
{
    ...
    const auto a = bcd_root_lower_bound(buffer1, v, l1);
    const auto b = std::get<0>(bcd_root_upper_bound(buffer1, v, 0.0));
    iters = 0;
    const auto phi = [&](auto h) {
        return bcd_root_function(h, buffer1, v, l1);
    };
    brent(
