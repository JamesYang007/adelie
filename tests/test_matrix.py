...
def test_naive_dense():
    def _test(n, p, dtype, order, seed=0):
        ...
        for dtype in dtypes:
            ...
            wrap = mod.pin_naive_dense(X, n_threads=4)
            cX = wrap._core_mat
...
def test_cov_dense():
    def _test(n, p, dtype, order, seed=0):
        ...
        for dtype in dtypes:
            ...
            wrap = mod.pin_cov_dense(A, n_threads=4)
            cA = wrap._core_mat
            ...
            # test diag
            out = np.array([cA.diag(i) for i in range(p)])
            assert np.allclose(np.diag(A), out, atol=atol)
...
def test_cov_lazy():
    def _test(n, p, dtype, order, seed=0):
        ...
        for dtype in dtypes:
            ...
            X = np.array(X, dtype=dtype, order=order)
            A = np.array(A, dtype=dtype, order=order)
            wrap = mod.pin_cov_lazy(X, n_threads=4)
            cA = wrap._core_mat
            ...
            # test coeff
            out = np.array([cA.diag(i) for i in range(p // 2)])
            assert np.allclose(np.diag(A[:p//2, :p//2]), out, atol=atol)
